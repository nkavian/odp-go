...
func TestMixedSuggestionsFilters(t *testing.T) {
	requests := 0
	value := client(t, func(request *http.Request) (*http.Response, error) {
		requests++
		var payload directory.SuggestionRequest
		if err := json.NewDecoder(request.Body).Decode(&payload); err != nil {
			t.Fatal(err)
		}
		if request.Method != "POST" || request.Header.Get("Content-Type") != "application/json" || payload.Filters == nil || strings.Join(payload.Filters.Keywords, ",") != "weather" {
			t.Fatalf("request=%s payload=%+v", request.Method, payload)
		}
	...
	if _, err := value.Suggest(t.Context(), directory.SuggestionRequest{Prefix: "we", Filters: &directory.ServiceFilters{Keywords: []string{""}}}); err == nil {
		t.Fatal("invalid filters accepted")
	}
	if _, err := value.SuggestServices(t.Context(), directory.SuggestionRequest{Prefix: "we", Filters: filters}); err == nil {
		t.Fatal("Service-only filters accepted")
	}
	if requests != 1 || filters.Keywords[0] != "weather" {
		t.Fatalf("requests=%d filters=%+v", requests, filters)
	}
}
...
func TestMixedSuggestionsUseSeparateEndpoint(t *testing.T) {
	...
	value := client(t, func(request *http.Request) (*http.Response, error) {
		...
		if request.URL.Path == "/v1/directory/suggestions" {
			var payload directory.SuggestionRequest
			if err := json.NewDecoder(request.Body).Decode(&payload); err != nil || payload.Prefix != "we" || payload.Limit != 10 || request.Method != "POST" {
				t.Fatalf("request=%s %s payload=%+v err=%v", request.Method, request.URL, payload, err)
			}
		} else if request.Method != "GET" || request.URL.Query().Get("prefix") != "we" || request.URL.Query().Get("limit") != "10" {
			t.Fatalf("request=%s %s", request.Method, request.URL)
		}
